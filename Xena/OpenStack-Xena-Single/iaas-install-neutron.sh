...
set -ex
source openrc.sh
if [[ ! -n "$(echo $INTERFACE_NAME)" ]];then
    echo INTERFACE_NAME none
    exit 1
fi
...
crudini --set /etc/nova/nova.conf neutron auth_type v3password
...
nmcli c reload $INTERFACE_NAME
nmcli d reapply $INTERFACE_NAME
# 落定服务
